...
- **The background bed's level is per-job, not a preference.** `bg_volume` (ceiling when nobody speaks) and `bg_ducking` are read from the ctx by `_stage_merge` and fall back to the global Output settings. They are in `_RETRY_OVERRIDE_KEYS` and `STAGE_RETRY_OPTIONS["merge"]`, which is what makes "the music is too quiet" fixable on a finished dub: `merge` is the last stage and a pure mux, so re-running it re-mixes in seconds and leaves the synthesized voices untouched.
