...
- **CLI**: `tools/gochidubb_cli.py` (31 subcommands: `dub`, `compare`, `showcase`, `redub`, `status`, `jobs`, `wait`, `showcase-status`, `showcase-rebuild`, `cast`, `continue`, `retry-stage`, `flags`, `edit-translations`, `glossary-term`, `quality`, `audit`, `publish`, `approve`, `publish-cancel`, `publish-inbox`, `scout`, `scout-dub`, `check-dup`, `cancel`, `rescue`, `delete`, `system`, `languages`, `models`, `voices`) — all HTTP calls via `tools/gochidubb_client.py`
- **MCP**: `tools/gochidubb_mcp.py` exposes 32 `gochidubb_*` tools mirroring the CLI: dub/compare/showcase/redub, job status & listing, voice casting (get/set/preview), continue/cancel/rescue/delete, per-stage retry, review flags, translation edits, glossary terms, quality report, artifact audit, publish workflow (stage/approve/cancel/pending), trending scout, duplicate check, and system/languages/models/voices — the `async def gochidubb_*` defs in that file are the authoritative list
...
- **The background bed's level is per-job, not a preference — and its scale runs past unity on purpose.** `bg_volume` is a raw ffmpeg gain on the separated stem; 1.0 is the stem at its ORIGINAL level, which sits ~10 dB under the loudnorm-hot dub voice (measured: source music window −20.5 LUFS, unity bed buried; ×10 bed −10.6 LUFS restores the source's voice:music balance). Hence the 0–10 range, default 10, `bg_ducking` off by default, and the `alimiter` closing both mix graphs — don't "clean up" the >1.0 range back to a 0–1 slider. `bg_volume`/`bg_ducking` are read from the ctx by `_stage_merge` AND `_run_tts_and_merge_stage`, falling back to the global Output settings; they are in `_RETRY_OVERRIDE_KEYS` and `STAGE_RETRY_OPTIONS["merge"]`, which is what makes "the music is too quiet" fixable on a finished dub: `merge` is the last stage and a pure mux, so re-running it re-mixes in seconds and leaves the synthesized voices untouched. The `/qc` "Background bed" row measures the dub-vs-source delta over the longest speech-free window (`pipeline/quality.py::bed_balance`).
